  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
              ...
              ? pageEntries
                  .map(
                    (e, i) =>
                      `**${start + i + 1}.** [${new Date(
                        e.createdAt
                      ).toLocaleDateString()}] ${e.entry.slice(0, 35)}`
                  )
                  .join("\n")
              : "No entries found."
          );
      ...
      if (
        ...
        if (sentDate !== today) {
          const embed = new EmbedBuilder()
            .setDescription(
              "⏳ You cannot mark off a habit from a previous day!"
            )
            .setColor(0x663399);
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith("habit_modal_create_")) {
        ...
        const description =
          interaction.fields.getTextInputValue("habit_description");
        const hour = parseInt(
          interaction.fields.getTextInputValue("habit_hour"),
          10
        );
        ...
        if (frequency === "weekly" && !validDays.includes(dayOfWeek)) {
          await interaction.reply({
            content:
              'Invalid day of week! Please enter a valid day, e.g., "Monday".',
            ephemeral: true,
          });
          return;
        }
