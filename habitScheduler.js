...
function scheduleHabitReminder(client, habit) {
  ...
  if (msUntil < 0) {
    console.error(
      `[HabitScheduler] Negative msUntil for habit ${habit._id}. Skipping scheduling.`
    );
    return;
  }
  ...
  scheduledHabits[habit._id] = setTimeout(async () => {
    ...
    try {
      ...
      const buttons = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`habit_dm_${habit._id}_${todayISO}_yes`)
          .setLabel("Yes")
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId(`habit_dm_${habit._id}_${todayISO}_nottoday`)
          .setLabel("Not Today")
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
          .setCustomId(`habit_dm_${habit._id}_${todayISO}_skip`)
          .setLabel("Skip")
          .setStyle(ButtonStyle.Secondary)
      );
      ...
      await user.send({
        embeds: [
          new EmbedBuilder()
            .setTitle(
              `<:pcht1:1391606087245103225> Habit Reminder: ${latestHabit.name}`,
            )
            .setDescription(
      ...
      console.log(
        `[HabitScheduler] Sent habit reminder to ${user.id} for "${latestHabit.name}"`
      );
    } catch (e) {
  ...
  console.log(
    `[HabitScheduler] Scheduled habit "${habit.name}" (${habit._id}) for user ${
      habit.userId
    } in ${Math.round(msUntil / 1000)} seconds`
  );
}
