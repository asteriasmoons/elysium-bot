...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('Announce that you purchased a book from someone\'s gift list.') // Command description
        // .setDMPermission(false) // *** REMOVED THIS LINE AS REQUESTED ***
        .addUserOption(option => // Option for the recipient user
            option.setName('recipient')
                .setDescription('The user who received the gifted book.')
                .setRequired(true)
        )
        .addStringOption(option => // Option for the book title
            option.setName('book_title')
                .setDescription('The title of the gifted book.')
                .setRequired(true)
        )
        .addStringOption(option => // Option for the author
    ...
    async execute(interaction) {
        ...
        if (!interaction.guild) {
            ...
            await interaction.reply({ content: 'This command needs to be used in a server to announce the gift in the designated channel.', ephemeral: false });
            return;
        }
        ...
        try {
            ...
            if (!settings || !settings.giftChannelId) {
                await interaction.reply({
                    content: 'The Gift Announcement Channel has not been set for this server. An admin needs to set it using `/channel set gifts`.',
                    ephemeral: false
                });
                return;
            }
            ...
            const giftsChannel = interaction.guild.channels.cache.get(settings.giftChannelId);

            // Check if the channel was found (it might have been deleted after being set)
            if (!giftsChannel) {
                await interaction.reply({
                    content: 'The designated Gift Announcement Channel was not found. Please ask an admin to set a new channel using `/channel set gifts`.',
                    ephemeral: false
                });
                return;
            }
            ...
            const botPermissions = giftsChannel.permissionsFor(interaction.client.user);
            ...
            console.error(`Error announcing gifted book for guild ${interaction.guild?.id}:`, error); // Added ?. for safety
            await interaction.reply({ content: 'There was an error trying to announce the gifted book.', ephemeral: false });
        }
    },
};