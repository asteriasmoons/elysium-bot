...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("view").setDescription("View all shop items")
    )
    .addSubcommand((sub) =>
      sub
        .setName("buy")
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addIntegerOption((opt) =>
          opt.setName("price").setDescription("XP cost").setRequired(true)
        )
        .addIntegerOption((opt) =>
          opt
            .setName("stock")
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("field")
            .setDescription(
              "The field to edit (name, description, price, stock, emoji)"
            )
            .setRequired(true)
            .addChoices(
  ...
  async execute(interaction) {
    ...
    if (sub === "buy") {
      ...
      if (item.stock !== null && item.stock < quantity) {
        return interaction.reply({
          content: `Not enough stock. Only ${item.stock} left.`,
          ephemeral: false,
        });
      }
      ...
      if (user.xp < totalCost) {
        return interaction.reply({
          content: `You need **${totalCost} XP** to buy ${quantity}x ${item.name}, but you only have **${user.xp} XP**.`,
          ephemeral: false,
        });
      }
      ...
      if (item.name.toLowerCase() === "unlimited journal entries") {
        if (user.hasUnlimitedJournal) {
          return interaction.reply({
            content: "You already own unlimited journal entries!",
            ephemeral: false,
          });
        }
        ...
        return interaction.reply({
          content: "You have unlocked unlimited journal entries!",
          ephemeral: false,
        });
      }
      ...
      return interaction.reply({
        content: `You bought **${quantity}x ${item.emoji || ""} ${
          item.name
        }** for **${totalCost} XP**!`,
        ephemeral: false,
      });
    ...
    if (sub === "add") {
      if (!ADMIN_IDS.includes(userId)) {
        return interaction.reply({
          content: "You do not have permission to add shop items.",
          ephemeral: false,
        });
      }
      ...
      if (exists) {
        return interaction.reply({
          content: "An item with that name already exists in the shop.",
          ephemeral: false,
        });
      }
      ...
      return interaction.reply({
        content: `Added **${emoji ? emoji + " " : ""}${name}** to the shop!`,
        ephemeral: false,
      });
    }
    ...
    if (sub === "remove") {
      if (!ADMIN_IDS.includes(userId)) {
        return interaction.reply({
          content: "You do not have permission to remove shop items.",
          ephemeral: false,
        });
      }
      const name = interaction.options.getString("item");
    ...
    if (sub === "edit") {
      if (!ADMIN_IDS.includes(userId)) {
        return interaction.reply({
          content: "You do not have permission to edit shop items.",
          ephemeral: false,
        });
      }
      const name = interaction.options.getString("item");
      ...
      if (field === "price" || field === "stock") {
        if (isNaN(Number(value))) {
          return interaction.reply({
            content: `❌ \`${field}\` must be a number.`,
            ephemeral: false,
          });
        }
      ...
      await item.save();
      return interaction.reply({
        content: `Updated **${field}** of **${item.name}** to **${value}**.`,
        ephemeral: false,
      });
    }
    ...
    // /shop inventory
    if (sub === "inventory") {
      let inventory = await UserInventory.findOne({ userId }).populate(
        "items.itemId"
      );
