const { SlashCommandBuilder, PermissionsBitField, EmbedBuilder, SlashCommandSubcommandGroupBuilder, SlashCommandSubcommandBuilder, SlashCommandChannelOption, ChannelType, SlashCommandStringOption } = require('discord.js');
  ...
  data: new SlashCommandBuilder()
      ...
      .addSubcommandGroup(group =>
          group
              ...
              .addSubcommand(sub =>
                  sub.setName('gifts')
                      .setDescription('Set the channel for Book Fairy gift announcements.')
                      .addChannelOption(option =>
                          option.setName('channel')
                              .setDescription('The channel to designate for gift announcements.')
                              .setRequired(true)
      ...
      .addSubcommand(sub =>
          sub.setName('reset')
              ...
              .addStringOption(option =>
                  option.setName('type')
                      .setDescription('The type of channel setting to reset (optional, resets all if omitted).')
                      .setRequired(false)
                      .addChoices(
  ...
  async execute(interaction) {
      ...
      if (subcommandGroup === 'set') {
          ...
          try {
              ...
              console.error(`Error setting ${subcommand} channel for guild ${guildId}:`, error);
              await interaction.reply({ content: `There was an error setting the ${subcommand} channel.`, ephemeral: false });
          }
      ...
      // --- Handle 'view' subcommand ---
          ...
          try {
              ...
              if (settings) {
                  const sprintChannel = settings.sprintChannelId ? `<#${settings.sprintChannelId}>` : 'Not set';
                  const giftChannel = settings.giftChannelId ? `<#${settings.giftChannelId}>` : 'Not set';

                  embed.setDescription(`Here are the designated channels for this server:`)
                      .addFields(
                          { name: 'Sprint Channel', value: sprintChannel, inline: false },
                          { name: 'Gift Announcement Channel', value: giftChannel, inline: false }
                      );

              } else {
                  embed.setDescription('No designated channels have been set for this server yet.');
              }
              ...
              await interaction.reply({ content: 'There was an error fetching channel settings.', ephemeral: false });
          }
      ...
      // --- Handle 'reset' subcommand ---
          ...
          if (!channelTypeToReset) {
              ...
              successMessage = 'The Gift Announcement Channel setting has been cleared.';
          }
          ...
          try {
              ...
              console.error(`Error resetting ${channelTypeToReset || 'all'} channels for guild ${guildId}:`, error);
              await interaction.reply({ content: `There was an error resetting channel settings.`, ephemeral: false });
          }
      }
