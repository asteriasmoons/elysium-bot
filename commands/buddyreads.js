...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("search")
        .setDescription(
          "Search for buddy read partners by book and audience (cross-server)."
        )
        .addStringOption((option) =>
          option
            .setName("book")
        ...
        .addStringOption((option) =>
          option
            .setName("audience")
            .setDescription("Audience filter")
            .setRequired(false)
            .addChoices(
              { name: "All Ages", value: "All Ages" },
              { name: "Teen/YA", value: "Teen/YA" },
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option
            .setName("book")
            .setDescription("Book title (if you have multiple sessions)")
            .setRequired(false)
        )
    )
    // messages
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option
            .setName("book")
            .setDescription("Book title (optional)")
            .setRequired(false)
        )
    )
    // finish
    .addSubcommand((sub) =>
      sub
        .setName("finish")
        .setDescription(
          "Mark a BuddyRead session as finished for both participants."
        )
        .addStringOption((option) =>
          option.setName("book").setDescription("Book title").setRequired(true)
        )
    ...
    .addSubcommand((sub) =>
      sub
        .setName("leave")
        .setDescription(
          "Leave a BuddyRead session (the other user will be notified)."
        )
        .addStringOption((option) =>
          option.setName("book").setDescription("Book title").setRequired(true)
        )
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("id")
            .setDescription(
              "The unique announcement ID (first 6 chars shown in /buddyread announcements)"
            )
            .setRequired(true)
        )
    )
      ...
      sub
        ...
        .addStringOption((opt) =>
          opt
            .setName("id")
            .setDescription(
              "The announcement ID to delete (see /buddyread announcements)"
            )
            .setRequired(true)
        )
    ),
  ...
  async execute(interaction) {
    ...
    if (sub === "announce") {
      ...
      const announcement = new BuddyReadAnnouncement({
        userId,
        username,
        book,
        audience,
    ...
    if (sub === "delete") {
      ...
      if (!/^[0-9a-fA-F]{3,24}$/.test(announcementId)) {
        return interaction.reply(
          buildEmbed({
            title: "Invalid ID",
            description:
              "Please provide a valid announcement ID (hex characters only).",
            footer: "BuddyRead Announcement",
            color: 0xb399ff,
            ephemeral: true,
          })
      ...
      const allAnnouncements = await BuddyReadAnnouncement.find({
        userId: interaction.user.id,
      });
      const matches = allAnnouncements.filter((a) =>
        a._id.toString().startsWith(announcementId)
      );
      ...
      if (matches.length > 1) {
        return interaction.reply(
          buildEmbed({
            title: "Ambiguous ID",
            description:
              "More than one announcement matches that ID. Please use more characters.",
            footer: "BuddyRead Announcement",
            color: 0xb399ff,
            ephemeral: true,
          })
    ...
    if (sub === "search") {
      ...
      if (!results.length)
        return interaction.reply(
          buildEmbed({
            ...
            description: `No buddy read announcements found for **${book}**${
              audience ? ` (${audience})` : ""
            }.`,
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields({
          name: "Important Info!",
          value:
            "In order to pair you have to have the ID of the announcement for the book you searched if you find matches. Use `/buddyread announcements` to see the ID of the announcement for the book",
          inline: false,
        })
        .setFooter({
          text: "Use the `/buddyread pair` command to pair with a user!",
        })
        .setTimestamp();
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          announcements
            .map(
              (a, i) =>
                `**${i + 1}. [ID: \`${a._id.toString().slice(0, 6)}\`] ${
                  a.book
                }**\n` +
                `**By:** ${a.username}\n**Audience:** ${a.audience}` +
                (a.note ? `\n**Note:** ${a.note}` : "") +
                `\n**Announced:** <t:${Math.floor(
                  new Date(a.createdAt || a._id.getTimestamp()).getTime() / 1000
                )}:d>`
            )
            .join("\n\n") +
    ...
    if (sub === "dm") {
      ...
      if (sessions.length === 1) session = sessions[0];
        ...
        session = sessions.find(
          (s) => s.book.toLowerCase() === bookTitle.toLowerCase()
        );
        if (!session)
          return interaction.reply(
            buildEmbed({
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          .setColor(0xb399ff)
          .setTitle("<:xmail:1368803966304911371> BuddyRead Message")
        ...
        await interaction.reply(
          buildEmbed({
            ...
            description:
              "Your message has been sent to your buddyread partner!",
    ...
    if (sub === "messages") {
      ...
      const messages = await BuddyReadMessage.find({
        sessionId: session._id,
      }).sort({ sentAt: 1 });
      if (!messages.length)
        return interaction.reply(
      ...
      const history = messages
        ...
        .map(
          (m) =>
            `**${m.senderTag}:** ${m.content} _(${m.sentAt.toLocaleString()})_`
        )
        .join("\n");
      await interaction.reply(
        buildEmbed({
          title: `<:xmail:1368803966304911371> BuddyRead Message History (${session.book})`,
          description: history,
        ...
        .setDescription(
          sessions
            .map((s, i) => {
              ...
              return `**${i + 1}. ${s.book}**\n**Buddy:** ${
                buddy ? buddy.username : "N/A"
              }\n**Status:** ${
                s.status.charAt(0).toUpperCase() + s.status.slice(1)
              }\n**Started:** <t:${Math.floor(
                new Date(s.startedAt).getTime() / 1000
              )}:d>`;
            })
            .join("\n\n")
        )
      ...
      const embeds = sessions.map((session) => {
        ...
        return new EmbedBuilder()
          ...
          .addFields(
            {
              name: "Buddy",
              value: buddy ? buddy.username : "N/A",
              inline: false,
            },
    ...
    if (sub === "finish") {
      const userId = interaction.user.id;
      const book = interaction.options.getString("book");
      const session = await BuddyReadSession.findOne({
        status: "active",
        "participants.userId": userId,
      ...
      if (buddy) {
        ...
        try {
          ...
          const leftEmbed = new EmbedBuilder()
            ...
            .setDescription(
              `Your BuddyRead partner (${interaction.user.tag}) has **left** your session for **${session.book}**.\n\nThe session is now unmatched.`
            )
    ...
    if (sub === "pair") {
      try {
        ...
        const announcement = announcements.find((a) =>
          a._id.toString().startsWith(inputId)
        );
        if (!announcement)
          return interaction.reply({
        ...
        const existingSession = await BuddyReadSession.findOne({
          ...
          "participants.userId": {
            $all: [announcement.userId, interaction.user.id],
          },
          status: "active",
        });
        if (existingSession)
        ...
        try {
          const announcer = await interaction.client.users.fetch(
            announcement.userId
          );
        ...
        const errorEmbed = new EmbedBuilder()
          ...
          .setDescription(
            "An error occurred while pairing. Please contact an admin."
          )
          .setTimestamp();
        return interaction.reply({
          embeds: [errorEmbed],
